...
export default function ModelView(props: { modelId: string }) {
  ...
  const generateImage = async () => {
    ...
    try {
      const resData = await fetch(`/api/models/${props.modelId}`, {
        method: "POST",
        ...
        <div className="relative w-[512px] h-[512px]">
          <div className="absolute flex flex-row bottom-0 right-0 mb-4 mr-3 items-center">
            <a href={imageUrl} download={"image.jpg"}>
              <Button disableRipple className=" bg-transparent mb-1" isIconOnly>
